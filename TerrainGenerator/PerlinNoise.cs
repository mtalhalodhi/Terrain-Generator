...
namespace TerrainGenerator
{
    static class PerlinNoise
    {
        ...
        public static double OctavePerlin(double x, double y, double z, double scale, int octaves, double persistence, double lacunarity, int seed)
        {
            ...
            {
                total += Perlin(x / scale * frequency + o, y / scale * frequency, z * frequency) * amplitude;
        ...
        public static double[,] GenerateNoiseMap(int width, int height, double scale, int octaves, double persistance, double lacunarity, int seed)
        {
            ...
            for(int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    double perlin = OctavePerlin(sampleX, sampleY, 1, scale, octaves, persistance, lacunarity, seed);
                    map[x, y] = perlin;

                    if (perlin < min) min = perlin;
                    if (perlin > max) max = perlin;
                }
            }
